...
Grid *create_grid(int N, double *points)
{
...
void compute_grid_midpoints(Grid *grid, int N, double *points, double *grid_midpoints)
{
    ...
    grid_midpoints[grid->N] = (grid->grid_right + grid->grid_points[grid->N - 1]) / 2;
}
...
void destroy_grid(Grid *grid_to_be_freed)
{
