...
def run_test(Lambda):
    ...
    flow = Flow(spatial_dimension, Lambda, kir, grid, mu, T,
                n_flavor, save_flow_flag=True, console_logging=False, number_of_observables=500, tolerance=1e-12)
    print(flow)
...
def main():
    Lambda_array = np.geomspace(1e5, 1e4, 10)

    job_list = (delayed(run_test)(Lambda) for Lambda in Lambda_array)
